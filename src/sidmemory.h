...
public:
	...
	*/
	virtual uint8_t readMemByte ( uint16_t addr ) = 0;
	...
	* @param addr the memory location from which to read from
	*/
	virtual uint16_t readMemWord ( uint16_t addr ) = 0;
	...
	*/
	virtual void writeMemByte ( uint16_t addr, uint8_t value ) = 0;
	...
	* @param value the value to write
	*/
	virtual void writeMemWord ( uint16_t addr, uint16_t value ) = 0;
	...
	*/
	virtual void fillRam ( uint16_t start, uint8_t value, unsigned int size ) = 0;
	...
	*/
	virtual void fillRam ( uint16_t start, const uint8_t* source, unsigned int size ) = 0;
	...
	*/
	virtual void installResetHook ( uint16_t addr ) = 0;
	...
	* @param addr the new addres to point to
	*/
	virtual void installBasicTrap ( uint16_t addr ) = 0;
