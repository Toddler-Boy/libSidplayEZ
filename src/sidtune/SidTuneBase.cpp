...
SidTuneBase* SidTuneBase::getFromBuffer ( const uint8_t* const buffer, uint32_t bufferLen )
{
...
void SidTuneBase::acceptSidTune ( const char* dataFileName, const char* infoFileName, buffer_t& buf, bool isSlashedFileName )
{
	// Make a copy of the data file name and path, if available
	if ( dataFileName )
	{
		const auto	fileNamePos = isSlashedFileName ?
	...
	// Calculate any remaining addresses and then confirm all the file details are correct
	...
	if ( info.m_dataFileLen >= 2 )
	{
		...
		info.m_fixLoad = get_little16 ( &buf[ fileOffset ] ) == ( info.m_loadAddr + 2 );
	}
...
void SidTuneBase::convertOldStyleSpeedToTables ( uint32_t speed, SidTuneInfo::clock_t clock )
{
	...
	const auto	toDo = std::min ( info.m_songs, MAX_SONGS );
	for ( auto s = 0u; s < toDo; s++ )
	{
		clockSpeed[ s ] = clock;
...
bool SidTuneBase::checkRelocInfo ()
{
	...
	const auto	startp = info.m_relocStartPage;
	const auto	endp = uint8_t ( ( startp + info.m_relocPages - 1 ) & 0xff );
	if ( endp < startp )
		return false;

	{
		// Check against load range
		const auto	startlp	= uint8_t ( info.m_loadAddr >> 8 );
		const auto	endlp	= startlp + uint8_t ( ( info.m_c64dataLen - 1 ) >> 8 );
...
void SidTuneBase::resolveAddrs ( const uint8_t* c64data )
{
	...
	if ( info.m_loadAddr == 0 )
	{
		...
		info.m_loadAddr = get_little16 ( c64data );
		fileOffset += 2;
		info.m_c64dataLen -= 2;
	}
