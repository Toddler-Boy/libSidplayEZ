	...
	, c ( &params[ 0 ] )
{
	...
	const auto	coeffLength = input.size () - 1;
	...
	for ( size_t i = 0; i < coeffLength; i++ )
	{
		...
		const auto	dx = input[ i + 1 ].x - input[ i ].x;
		const auto	dy = input[ i + 1 ].y - input[ i ].y;
	...
	for ( size_t i = 1; i < coeffLength; i++ )
	{
		...
		{
			const auto	dx = dxs[ i - 1 ];
			const auto	dxNext = dxs[ i ];
			const auto	common = dx + dxNext;
	...
	for ( size_t i = 0; i < coeffLength; i++ )
	{
		...
		const auto	c1 = params[ i ].c;
		const auto	m = ms[ i ];
		const auto	invDx = 1.0 / dxs[ i ];
		const auto	common = c1 + params[ i + 1 ].c - m - m;
